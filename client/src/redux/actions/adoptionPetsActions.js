...
export const addAdoptionPet = (adoptionPetData) => async (dispatch) => {
  ...
  const res = await fetch(`${API_URL}/pets/adoption`, {
    method: "POST",
    headers: {
...
export const adoptPet = (petID) => async (dispatch) => {
  ...
  const cookies = new Cookie();
  const token = cookies.get("authToken");
  const res = await fetch(`${API_URL}/pets/adoption/${petID}/adopt`, {
    method: "PUT",
    headers: {
      Authorization: `Bearer ${token}`,
      "Content-Type": "application/json",
...
export const resetAddAdoptionPetState = () => ({
  type: ADD_ADOPTION_PET_RESET,
});
